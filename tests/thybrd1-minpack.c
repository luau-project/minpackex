...
void minpack_hybrd1_fcn(int *n, double *x, double *fvec, int *iflag)
{
    /* function minpack_hybrd1_fcn for hybrd1 example */
...
void thybrd1_minpack_write_content(FILE *file)
{
    ...
    hybrd1_(&minpack_hybrd1_fcn, &n, x, fvec, &tol, &info, wa, &lwa);
    fnorm = enorm_(&n, fvec);
